...
export function isEmbeddableImage(file: File, maxImageSize?: number): ImageCheck {
 ...
 */
export async function resolveImageSource(file: File, options: ImageEmbedOptions = {}): Promise<string> {
  if (options.onImageUpload) return options.onImageUpload(file);
