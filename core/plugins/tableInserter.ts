...
export const tablePlugin: EditorPlugin = {
  ...
  toolbarButtons: [
    {
      ...
      onClick: (context: EditorContext) => {
        ...
        const attachTableControls = (wrapper: HTMLElement) => {
          const table = wrapper.querySelector('.lilac-table') as HTMLTableElement;
          if (!table) return;
              ...
              switch (action) {
                ...
                case 'add-col': {
                  table.querySelectorAll('tr').forEach((row, idx) => {
                    const cell = document.createElement(idx === 0 && table.querySelector('thead') ? 'th' : 'td');
                    cell.contentEditable = 'true';
        ...
        const updatePreview = () => {
          ...
          const wrapper = previewContainer.querySelector('.lilac-table-wrapper') as HTMLElement;
          if (wrapper) {
        ...
        insertBtn.addEventListener('click', () => {
          ...
          // Attach controls to inserted table
          setTimeout(() => {
            const wrappers = context.element?.querySelectorAll('.lilac-table-wrapper');
            if (wrappers) {
              wrappers.forEach(wrapper => {
                attachTableControls(wrapper as HTMLElement);
              });
            }
          }, 100);
        });
  ...
  styles: `
    ...
    .lilac-btn-secondary {
      background: var(--lilac-color-surface, #f8f9fb);
      color: var(--lilac-color-text, #2c3e50);
      border: 1px solid var(--lilac-color-border, #e1e5e9);
    }
