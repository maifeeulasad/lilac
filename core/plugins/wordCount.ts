...
function calculateStats(content: string): WordCountState {
...
function createWordCountPanel(context: EditorContext): HTMLElement {
  ...
  panels: [
    {
      ...
      position: 'right',
      defaultOpen: false,
      render: (context: EditorContext, container: HTMLElement) => {
        ...
        (panel as any)._updateInterval = interval;
      },
      destroy: () => {
        // Cleanup any intervals
        const panel = document.querySelector('.lilac-word-count-panel') as any;
  ...
  toolbarButtons: [
    {
      ...
      onClick: (context: EditorContext) => {
        let modal = document.querySelector('.lilac-word-count-modal') as HTMLElement;
        if (modal) {
          modal.remove();
          return;
        }
