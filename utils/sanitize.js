...
function stripUrlNoise(url) {
    ...
    for (const ch of url) {
        ...
        if (code <= 0x20)
            continue; // C0 controls and space
        if (code === 0xa0 || code === 0x1680)
            continue; // NBSP, Ogham space mark
        if (code >= 0x2000 && code <= 0x200d)
            continue; // en/em spaces, zero-width
        if (code === 0x2028 || code === 0x2029)
            continue;
        if (code === 0x205f || code === 0x3000)
            continue;
        if (code === 0xfeff)
            continue; // BOM
        out += ch;
...
export function isSafeUrl(url, allowDataImage = false) {
    ...
    if (allowDataImage && SAFE_DATA_IMAGE.test(normalized))
        return true;
